...
const UserChats = ({ data }) => {
    return (
        <div className='flex flex-col w-full gap-4 p-6 h-full overflow-y-scroll'>
            {
                data.chat1.map((item, id) => {
                    ...
                    if (id % 2 == 0) {
                        return <div key={id} className='w-full flex items-center justify-start'>
                            {
                                item.msg.message.length !== 0 && <h1 className='w-[60%] bg-gray-100 p-4 text-[1rem] rounded-tr-md rounded-tl-md rounded-br-md'> {item.msg.message}</h1>
                            }
                        </div>
                    } else {
